...
The `ranges` subpackage bundles the published IP ranges of some well-known reverse-proxy providers (`ranges.Cloudflare`, `ranges.CloudFront`) for use with `realclientip.RightmostTrustedRangeStrategy`. Contributions are welcome to add new providers.

> [!WARNING]
> These lists are **static snapshots** and *will* drift from the providers' live ranges. Trusting a stale list with `RightmostTrustedRangeStrategy` is a security risk in both directions: a range the provider has **released** may be reassigned to a third party who is then wrongly trusted (enabling IP spoofing), and a range the provider has **added** is treated as untrusted (yielding a wrong or empty result). For correctness-critical use, retrieve the ranges at runtime from the provider ([Cloudflare API](https://api.cloudflare.com/#cloudflare-ips-properties), AWS [`origin-facing` managed prefix list](https://docs.aws.amazon.com/AmazonCloudFront/latest/DeveloperGuide/LocationsOfEdgeServers.html)), or verify proxy identity by a means stronger than IP (e.g. authenticated origin pulls).

The bundled files are generated by `go generate ./ranges/...` and a weekly CI job (`.github/workflows/ranges-drift.yml`) opens a PR when they drift upstream. Even so, there is no freshness guarantee at any given commit.
